...
const ComplaintTableRow = ({ complaint, onView }: Props) => {
  ...
  return (
    <tr className="hover:bg-slate-100 transition-colors">
      <td className="pl-4 pr-6 py-4 text-slate-400 text-xs font-medium">
        #{complaintId ?? "NA"}
      </td>
      ...
      <td className="px-2 py-4">
        <span className="bg-slate-100 text-slate-600 text-xs px-2.5 py-1 whitespace-nowrap rounded-full font-medium">
          {SUBJECT_LABELS[subject] ?? subject ?? "NA"}
        </span>
      </td>
      ...
      <td className="pl-4 pr-6 py-4">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-full bg-indigo-100 text-indigo-600 text-xs font-semibold flex items-center justify-center flex-shrink-0">
            {customer.name.charAt(0).toUpperCase() ?? "NA"}
          </div>
          <span className="text-slate-700 text-sm">{customer.name ?? "NA"}</span>
        </div>
      </td>
      ...
      <td className="pl-4 py-4 text-slate-400 text-xs">
        {formatDate(createdAt) ?? "NA"}
      </td>
