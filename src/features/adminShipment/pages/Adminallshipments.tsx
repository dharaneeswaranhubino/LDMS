  ...
  const kpi = useMemo(
    () => ({
      ...
      delivered: shipments.filter(
        (s) =>
          ["DELIVERED", "CANCELLED"].includes(s.shipmentStatus) === false &&
      ...
      {kpi.delayed > 0 && (
        <div className="flex items-center gap-3 bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 mb-4">
          ...
          <p className="text-[12px] text-orange-700">
            <strong>
              {kpi.delayed} shipment{kpi.delayed > 1 ? "s" : ""} delayed
            </strong>{" "}
            ...
            <thead className="bg-slate-50 border-b border-slate-200 sticky top-0">
              <tr>
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-4 py-3 text-[10px] font-600 text-slate-500 uppercase tracking-wide whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-semibold flex-shrink-0 ${avatarColor(s.customerName)}`}
                        >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-1 rounded-lg text-[10px] font-semibold border ${PRIORITY_STYLE[s.shipmentPriority]}`}
                      >
                    ...
                    <td className="px-4 py-3">
                      ...
                      <p
                        className={`text-[10px] mt-0.5 ${s.paymentStatus === "PAID" ? "text-green-600" : s.paymentStatus === "FAILED" ? "text-red-500" : "text-sky-600"}`}
                      >
                        {s.paymentStatus}
                      </p>
                    </td>
            ...
            <div className="mb-5">
              ...
              >
                ...
                {MOCK_AGENTS.map((agent) => (
                  ...
                  >
                    {agent.name} ({agent.todayCount}/8 today · {agent.status})
                    {agent.name === reassignModal.assignedAgentName
                      ? " — current"
                      : ""}
                  </option>
                ))}
              ...
              <p className="text-[11px] text-slate-400 mt-1.5">
                ...
                Backend will auto-assign the next available slot for the
                selected agent.
              </p>
            </div>
        ...
        >
          ...
          >
            ...
            <p className="text-[12px] text-slate-400 text-center mb-5">
              This will close the shipment permanently. Chat becomes read-only.
