...
import type { PriceBreakdownProps, RazorpayOptions, RazorpayResponse } from "../../shipmentTypes";
import { loadRazorpayScript, unloadRazorpayScript } from "../../utils/shipmentHelpers";
import { useNavigate } from "react-router-dom";
...
const PriceBreakdown = ({
  ...
  const { platformFee, weightCharge, priorityCharge, fragileCharge, subtotal, gst, total } = priceBreakdown;
  ...
  if (paymentStatus === "verifying") {
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-8 mt-4 flex flex-col items-center gap-4 shadow-sm">
        ...
        <p className="text-xl font-semibold text-slate-700">Verifying payment...</p>
        <p className="text-slate-500 text-sm text-center">Please wait while we confirm your payment securely.</p>
      </div>
    );
  }
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-5 mt-4 shadow-sm">
      ...
      <div className="mt-5">
        ...
        >
          ...
          {isProcessing ? "Processing..." : retryCount >= 3 ? "Payment limit reached" : "Proceed to payment"}
        </button>
      </div>

      <p className="text-[12px] text-slate-500 mt-3">
        Payment required before admin assignment. You can retry up to 3 times if payment fails.
      </p>
