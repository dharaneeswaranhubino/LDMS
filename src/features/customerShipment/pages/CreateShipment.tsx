...
import type { Address, PackageDetailsFormData, PriceBreakdown as PriceBreakdownType } from "../shipmentTypes";

interface CreatedShipmentMeta {
  shipmentId: number;
  trackingId: string;
...
const CreateShipment = () => {
  ...
  const [createdMeta, setCreatedMeta] = useState<CreatedShipmentMeta | null>(null);

  const [pickUpAddress, setPickUpAddress] = useState<Address>({
    name: "", phone: "", fullAddress: "", city: "", pinCode: "",
  });
  ...
  const [packageDetails, setPackageDetails] = useState<PackageDetailsFormData>({
    itemName: "", quantity: "", weight: "", description: "",
    fragile: false, priority: "STANDARD", deliveryFrom: "", deliveryTo: "",
  });
  ...
  const goPrev = () => { setDirection(-1); setStep((p) => p - 1); };

  // Called by PackageDetails "Next" button
  // Dispatches createShipment, stores meta, then advances step
    ...
    try {
      ...
      const result = await dispatch(
        createShipment({
          ...
          deliveryAddress,
          packageDetails,
          amount: 0, // backend calculates — remove this field once Suji confirms not needed
        })
      ).unwrap();
  ...
  const onReset = () => {
    ...
    setDirection(1);
    setCreatedMeta(null);
    setPickUpAddress({ name: "", phone: "", fullAddress: "", city: "", pinCode: "" });
    setDeliveryAddress({ name: "", phone: "", fullAddress: "", city: "", pinCode: "" });
    setPackageDetails({
      itemName: "", quantity: "", weight: "", description: "",
      fragile: false, priority: "STANDARD", deliveryFrom: "", deliveryTo: "",
    });
  ...
  return (
    <>
      <div className="rounded-2xl min-h-screen bg-gradient-to-br from-slate-50 via-sky-200 to-purple-50 p-3 overflow-hidden">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-slate-800">Send new shipment</h1>
            <p className="text-[14px] text-slate-500 mt-1">Fill in details below to create your shipment</p>
          </div>
          <div
            onClick={() => setIsInstruc(true)}
            className="cursor-pointer flex text-sky-900 items-center gap-1 p-2 bg-gradient-to-br from-slate-100 to-sky-100 rounded-lg border border-sky-200 hover:from-sky-100 hover:to-slate-100"
        ...
        <AnimatePresence mode="wait" custom={direction}>
          {step === 1 && (
            <motion.div key="shipment" custom={direction} variants={pageVariants}
              initial="initial" animate="animate" exit="exit" transition={{ duration: 0.4 }}>
              <ShipmentAddress
                nextStep={goNext}
                pickUpAddress={pickUpAddress}
                setPickUpAddress={setPickUpAddress}
          ...
          {step === 2 && (
            <motion.div key="package" custom={direction} variants={pageVariants}
              initial="initial" animate="animate" exit="exit" transition={{ duration: 0.4 }}>
              <PackageDetails
                onNext={handlePackageNext}   // ← async callback, not nextStep
                prevStep={goPrev}
          ...
          {step === 3 && createdMeta && (
            <motion.div key="price" custom={direction} variants={pageVariants}
              initial="initial" animate="animate" exit="exit" transition={{ duration: 0.4 }}>
              <PriceBreakdown
                prevStep={goPrev}
