...
import { setActiveShipment } from '../shipmentSlice';
...
const CustomerChat = () => {
  ...
  // Get customer's shipments from your existing shipmentSlice
  const shipments = useAppSelector((s) => s.shipment.shipments);

  // Only show shipments that have an assigned agent (can chat)
  const chattableShipments = shipments.filter(
    (s) => s.assignedAgent !== null && s.shipmentStatus !== 'DELIVERED' && s.shipmentStatus !== 'CANCELLED'
  );
  ...
  const activeShipment = chattableShipments.find((s) => s.shipmentId === activeShipmentId);
    ...
    <div className="flex h-[calc(100vh-72px)]">
      ...
      <div className="w-72 border-r border-gray-200 bg-white flex flex-col">
        <div className="p-4 border-b border-gray-200">
          ...
          <p className="text-xs text-gray-400 mt-1">Select a shipment to chat with your agent</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {chattableShipments.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 p-4 text-center">
              ...
              <p className="text-sm">No active shipments with assigned agents</p>
            </div>
          ) : (
            chattableShipments.map((shipment) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 border-b border-gray-100 hover:bg-gray-50 transition-colors ${
                  activeShipmentId === shipment.shipmentId ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''
                }`}
              >
        ...
        {activeShipmentId && currentUserId ? (
          <>
            ...
            <div className="px-6 py-4 border-b border-gray-200 bg-white">
              ...
              <p className="text-xs text-gray-400">Chat with your delivery agent</p>
            </div>
