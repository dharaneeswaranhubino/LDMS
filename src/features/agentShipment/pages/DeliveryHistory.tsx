...
const DeliveryHistory = () => {
  ...
  const filteredDeliveries = useMemo(() => {
    ...
    if (activeTab === "THIS_WEEK")  filtered = filtered.slice(0, 6);
    if (activeTab === "THIS_MONTH") filtered = filtered.slice(0, 10);
  ...
  const handleViewClick = (shipmentId: number) => {
    navigate("/deliveryDetail", { state: { shipmentId } });
  };
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row">
          <div className="relative flex items-center w-full">
            <i className="fa-solid fa-magnifying-glass absolute left-3 z-10 text-gray-500 pointer-events-none" />
            <input
              type="text"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[500px] overflow-y-scroll scrollbar-none rounded-2xl">
          {paginatedData.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white py-16">
              <i className="fa-solid fa-box-open text-5xl text-slate-300 mb-4" />
            ...
            paginatedData.map((item) => (
              <DeliveryCard
                key={item.shipmentId}
                item={item}
                onView={() => handleViewClick(item.shipmentId)}
              />
            ))
          )}
        </div>
